...
const renderHtml = (cartItems) => {
  let html = '<div class="section-line first-line"></div>'
          ...
          <div class="d-flex mobile-item">
            ...
            <div class="item__info">
              <div class="price-details price-details-mobile" >
                <h2 class="price">${Math.ceil(item.priceDiscount * item.inCart)}<span class="currency"> сом</span></h2>
                <div class="discount">
                  <span>${Math.ceil(item.price * item.inCart)} сом</span>
                </div>
              </div>
...
const cartDetails = () => {
  totalSum.textContent = new Intl.NumberFormat('ru-RU').format(String(Math.ceil(items.reduce((acc, item) => acc + item.priceDiscount * item.inCart, 0)))).replace(',', '.') + ' сом';
